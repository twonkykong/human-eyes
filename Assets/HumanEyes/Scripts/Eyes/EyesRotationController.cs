using HumanEyes.Scripts.Head;
using HumanEyes.Scripts.Movment;
using UnityEngine;
        ...
        private void CheckDistance()
        {
            if (!eyesRotation.IsRelaxed)
            {
                if (Physics.Linecast(rayStartPoint.position, viewTargetPoint.position, out RaycastHit hitInfo))
                {
                    ...
                    {
                        Vector2 objectSizeOnScreen =
                            ObjectSizeCalculator.CalculateObjectSizeOnCamera(hitInfo.transform.gameObject, eyeCamera);
